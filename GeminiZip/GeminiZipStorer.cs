...
namespace Universal {
    ...
    /// </summary>
    public class GeminiZipStorer : IDisposable {
        ...
        private readonly List<GeminiZipFileEntry> Files = new List<GeminiZipFileEntry>();
        // List of files in Central Directory
        private readonly List<GeminiZipFileEntry> CentralDirectoryFiles = new List<GeminiZipFileEntry>();
        ...
        public static GeminiZipStorer Create(string filename, string? comment = null) {
            Stream stream = new FileStream(filename, FileMode.Create, FileAccess.ReadWrite);

            GeminiZipStorer zip = Create(stream, comment);
        ...
        /// <returns>A valid ZipStorer object</returns>
        public static GeminiZipStorer Create(Stream stream, string? comment = null, bool leaveOpen = false) {
            GeminiZipStorer zip = new GeminiZipStorer() {
        ...
        /// <returns>A valid ZipStorer object</returns>
        public static GeminiZipStorer Open(string filename, FileAccess access) {
            Stream? stream = null;
            GeminiZipStorer? zip = null;
        ...
        /// <param name="leaveOpen">true to leave the stream open after the ZipStorer object is disposed; otherwise, false (default).</param>
        /// <returns>A valid ZipStorer object</returns>
        public static GeminiZipStorer Open(Stream stream, FileAccess access, bool leaveOpen = false) {
        ...
        public GeminiZipFileEntry AddFile(Compression method, string pathname, string filenameInZip, string? comment = null) {
        ...
        public GeminiZipFileEntry AddStream(Compression method, string filenameInZip, Stream source, DateTime modTime, string? comment = null) {
        ...
        /// <param name="comment">Comment for stored file</param>
        public async Task<GeminiZipFileEntry> AddStreamAsync(Compression method, string filenameInZip, Stream source, DateTime modTime, string? comment = null) {
            if (Access == FileAccess.Read) {
                throw new InvalidOperationException("Writing is not allowed");
            }
            // Prepare the fileinfo
            GeminiZipFileEntry zfe = new GeminiZipFileEntry() {
                Method = method,
        ...
        public List<GeminiZipFileEntry> ReadCentralDir(bool skipFileOffsetCalculation = false) {
            ...
            for (int pointer = 0; pointer < CentralDirImage.Length;) {
                ...
                DateTime modifyTimeDT = GeminiZipDate.DosTimeToDateTime(modifyTime) ?? DateTime.Now;
                EncodeUTF8 |= encodeUTF8;
        ...
        public bool ExtractFile(GeminiZipFileEntry zfe, string filename) {
        ...
        public bool ExtractFile(GeminiZipFileEntry zfe, Stream stream) {
        ...
        /// <returns>True if success, false if not.</returns>
        /// <remarks>Unique compression methods are Store and Deflate</remarks>
        public async Task<bool> ExtractFileAsync(GeminiZipFileEntry zfe, Stream stream) {
            if (!stream.CanWrite) {
        ...
        /// <returns>True if success, false if not.</returns>
        /// <remarks>Unique compression methods are Store and Deflate</remarks>
        public bool ExtractFile(GeminiZipFileEntry zfe, out byte[]? file) {
        ...
        public static bool RemoveEntries(ref GeminiZipStorer zip, List<GeminiZipFileEntry> zfes) {
        ...
        public List<GeminiZipFileEntry> GetEntries() {
            var list = new List<GeminiZipFileEntry>(CentralDirectoryFiles);
            list.AddRange(Files);
            return list;
        }
        /// <summary>
        ...
        public GeminiZipFileEntry? GetEntry(string name, StringComparison comparisonType = StringComparison.CurrentCultureIgnoreCase) {
        ...
        */
        private void writeLocalHeader(GeminiZipFileEntry zfe) {
            long pos = ZipFileStream!.Position;
            ...
            ZipFileStream!.Write(BitConverter.GetBytes(GeminiZipDate.DateTimeToDosTime(zfe.ModifyTime)), 0, 4); // zipping date and time
        ...
        */
        private void writeCentralDirRecord(GeminiZipFileEntry zfe) {
            Encoding encoder = zfe.EncodeUTF8 ? Encoding.UTF8 : DefaultEncoding;
            byte[] encodedFilename = encoder.GetBytes(zfe.FilenameInZip!);
        ...
        */
        ...
        private async Task<Compression> storeAsync(GeminiZipFileEntry zfe, Stream source) {
            if (source.Length == 0) {
                zfe.FileSize = 0;
            ...
            do {
                ...
                zfe.Crc32 = GeminiZipCrc32.UpdateCRC(zfe.Crc32, buffer, bytesRead);
        ...
        private void updateCrcAndSizes(GeminiZipFileEntry zfe) {
        ...
        private bool updateLocalHeaderExtraFields(GeminiZipFileEntry zfe) {
